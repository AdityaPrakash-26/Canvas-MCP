...
# No need to import file functions, we'll use the test_client


def test_get_course_files(test_client, target_course_info):
    ...
    # Get all files
    files = test_client.get_course_files(target_course_info["internal_id"])
    ...
    # Get PDF files
    pdf_files = test_client.get_course_files(
        target_course_info["internal_id"], file_type="pdf"
    )
...
def test_extract_text_from_course_file(test_client, target_course_info):
    ...
    if (
        "test_pdf_url" not in target_course_info
        or target_course_info["test_pdf_url"] is None
    ):
        pytest.skip("No PDF file URL available, skipping text extraction test.")

    # Extract text from the file
    result = test_client.extract_text_from_course_file(
        target_course_info["test_pdf_url"], file_type="pdf"
    )
