...
try:
    ...
    import subprocess
    subprocess.check_call([sys.executable, "-m", "uv", "add", "canvasapi", "python-dotenv"])
    from canvasapi import Canvas
...
API_URL = "https://canvas.instructure.com"  # Replace with your institution's Canvas URL
...
try:
    ...
    for course in courses:
        ...
        if hasattr(detailed_course, 'syllabus_body') and detailed_course.syllabus_body:
            print(f"  Syllabus available: {len(detailed_course.syllabus_body)} characters")
        else:
        ...
        try:
            ...
            for assignment in assignments:
                ...
                print(f"    Due: {assignment.due_at if hasattr(assignment, 'due_at') else 'No due date'}")
