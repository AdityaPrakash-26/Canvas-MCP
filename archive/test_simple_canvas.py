...
try:
    ...
    if term_ids:
        ...
        print(
            f"Found {len(current_courses)} courses in the most recent term (ID: {max_term_id})"
        )

        # Print course details
        for i, course in enumerate(current_courses[:5]):  # Show first 5
            print(
                f"  {i + 1}. {course.name} ({getattr(course, 'course_code', 'No code')}) - ID: {course.id}"
            )
        ...
        if current_courses:
            ...
            try:
                ...
                try:
                    ...
                    print(
                        f"Found {len(assignments)} assignments for course {detailed_course.name}"
                    )

                    # Print assignment details
                    for i, assignment in enumerate(assignments[:3]):  # Show first 3
                        print(
                            f"  {i + 1}. {assignment.name} - Due: {getattr(assignment, 'due_at', 'No due date')}"
                        )
                ...
                try:
                    ...
                    print(
                        f"Found {len(modules)} modules for course {detailed_course.name}"
                    )
