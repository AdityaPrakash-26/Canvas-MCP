...
def get_upcoming_deadlines(days: int = 7, course_id: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    params: List[Any] = [now.isoformat(), end_date.isoformat()]
...
@mcp.tool()
def search_course_content(query: str, course_id: Optional[int] = None) -> List[Dict[str, Any]]:
    """
    ...
    # Combine results
    results = assignments + modules + syllabi
