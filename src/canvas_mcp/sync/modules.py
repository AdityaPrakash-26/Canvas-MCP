...
async def sync_modules(sync_service: "SyncService", course_ids: list[int] | None = None) -> int:
    """
    ...
    logger.info(f"Creating tasks to fetch modules for {len(courses_to_sync)} courses...")
    ...
    module_results_or_exceptions = await asyncio.gather(*module_tasks, return_exceptions=True)
    ...
    raw_module_map: dict[int, Any] = {} # canvas_module_id -> raw_module object (for item fetching)
    ...
    for i, result in enumerate(module_results_or_exceptions):
        ...
        if isinstance(result, Exception):
            logger.error(f"Failed fetching modules for course {local_course_id} (Canvas ID: {canvas_course_id}): {result}")
            continue
        if result is None:
        ...
        for raw_module in raw_modules:
            try:
                ...
                if not canvas_module_id: continue
                ...
                module_data = {
                    ...
                    "unlock_at": getattr(raw_module, "unlock_at", None), # Alias for unlock_date
                    "position": getattr(raw_module, "position", None),
                    "require_sequential_progress": getattr(raw_module, "require_sequential_progress", False),
                }
                ...
                item_task = asyncio.create_task(
                    _fetch_module_items(sync_service, raw_module) # Pass raw module object
                )
                module_item_fetch_tasks.append(item_task)

            except Exception as e:
                logger.error(f"Validation error for module {getattr(raw_module, 'id', 'N/A')} in course {local_course_id}: {e}", exc_info=True)

    logger.info(f"Processed {total_raw_modules} raw modules, {len(all_valid_modules)} valid modules found.")

    # --- Parallel Fetch Stage (Module Items) ---
    logger.info(f"Gathering module item data for {len(module_item_fetch_tasks)} modules...")
    item_results_or_exceptions = await asyncio.gather(*module_item_fetch_tasks, return_exceptions=True)
    ...
    for i, result in enumerate(item_results_or_exceptions):
         ...
         if i < len(valid_module_canvas_ids):
             ...
             local_course_id = module_context_map.get(canvas_module_id) # Get course context
         else:
             logger.error(f"Index mismatch processing module item results ({i})")
             continue # Should not happen

         if isinstance(result, Exception):
             logger.error(f"Failed fetching items for module {canvas_module_id}: {result}")
             continue
         if result is None:
         ...
         for raw_item in raw_items:
             try:
                 ...
                 if not canvas_item_id: continue
                 ...
                 item_data = {
                     ...
                     "external_url": getattr(raw_item, "external_url", None), # Alias for url
                     "page_url": getattr(raw_item, "page_url", None),
                     # Content details might be large, consider storing selectively or hashing
                     "content_details": str(vars(raw_item)) if hasattr(raw_item, "__dict__") else None,
                 }
                 ...
                 logger.error(f"Validation error for item {getattr(raw_item, 'id', 'N/A')} in module {canvas_module_id}: {e}", exc_info=True)

    logger.info(f"Processed {total_raw_items} raw module items, {len(all_valid_module_items)} valid items found.")
    ...
    logger.info(f"Finished module sync. Persisted/updated {persisted_module_count} modules and their items.")
    return persisted_module_count


async def _fetch_modules_for_course(sync_service: "SyncService", canvas_course_id: int, local_course_id: int) -> list[Any] | None:
    """Helper async function to wrap the threaded API call for modules."""
    async with sync_service.api_semaphore:
        logger.debug(f"Semaphore acquired for fetching modules: course {local_course_id}")
        try:
            ...
            logger.debug(f"Fetched {len(raw_modules)} modules for course {local_course_id}")
            return raw_modules
        except Exception as e:
            logger.error(f"Error in thread fetching modules for course {local_course_id}: {e}", exc_info=True)
            return None

async def _fetch_module_items(sync_service: "SyncService", raw_module: Any) -> list[Any] | None:
    ...
    async with sync_service.api_semaphore:
        ...
        try:
            ...
            logger.error(f"Error in thread fetching items for module {module_id}: {e}", exc_info=True)
            return None
...
def _persist_modules_and_items(
    ...
    module_canvas_to_local_id: dict[int, int] = {} # canvas_module_id -> local_module_id

    # --- Module Persistence ---
    if valid_modules:
        # 1. Fetch existing module IDs
        existing_modules_map: dict[tuple[int, int], int] = {} # (canvas_module_id, course_id) -> local_module_id
        ...
        try:
            if canvas_ids_in_batch and course_ids_in_batch:
                ...
                for row in cursor.fetchall():
                    existing_modules_map[(row['canvas_module_id'], row['course_id'])] = row['id']
        ...
        for db_module in valid_modules:
            ...
            module_dict["unlock_date"] = module_dict.pop("unlock_at", db_module.unlock_date) # Handle alias
        ...
        if modules_to_insert_data:
            ...
            try:
                ...
                # Get local IDs for newly inserted modules
                last_row_id = cursor.lastrowid
                first_new_id = last_row_id - inserted_count + 1
                for i, data_tuple in enumerate(modules_to_insert_data):
                    canvas_id = data_tuple[1] # canvas_module_id
                    new_local_id = first_new_id + i
                    module_canvas_to_local_id[canvas_id] = new_local_id
            except sqlite3.Error as e:
        ...
        if modules_to_update_data:
            logger.debug(f"Updating {len(modules_to_update_data)} modules individually...")
            for item_dict in modules_to_update_data:
                ...
                try:
                    set_clause = ", ".join([f"{k} = ?" for k in item_dict if k not in ['canvas_module_id', 'course_id']])
                    values = [v for k, v in item_dict.items() if k not in ['canvas_module_id', 'course_id']]
                    values.append(local_id)
                    sql = f"UPDATE modules SET {set_clause} WHERE id = ?"
                    cursor.execute(sql, values)
                    update_count += cursor.rowcount
                except sqlite3.Error as e:
                    logger.error(f"Failed to update module {canvas_id} (local ID {local_id}) in course {course_id}: {e}")
    ...
    if valid_module_items:
        ...
        for db_item in valid_module_items:
            ...
            if not local_module_id:
                logger.warning(f"Skipping item {db_item.canvas_item_id}: Cannot find local module ID for canvas module {db_item.canvas_module_id}")
                continue

            item_dict = db_item.model_dump(exclude={"created_at", "updated_at", "canvas_module_id"}) # Exclude temp field
            item_dict["updated_at"] = now_iso
            item_dict["module_id"] = local_module_id # Set the correct foreign key
            item_dict["item_type"] = item_dict.pop("type", db_item.item_type) # Handle alias
            item_dict["url"] = item_dict.pop("external_url", db_item.url) # Handle alias
        ...
        if items_to_persist:
            logger.debug(f"Persisting {len(items_to_persist)} module items using INSERT OR REPLACE...")
