...
async def sync_assignments(sync_service: "SyncService", course_ids: list[int] | None = None) -> int:
    """
    ...
    logger.info(f"Creating tasks to fetch assignments for {len(courses_to_sync)} courses...")
    for canvas_course_id, local_course_id in courses_to_sync.items():
        task = asyncio.create_task(
            _fetch_assignments_for_course(sync_service, canvas_course_id, local_course_id)
        )
        tasks.append(task)
    ...
    for i, result in enumerate(results_or_exceptions):
        ...
        if isinstance(result, Exception):
            logger.error(f"Failed fetching assignments for course {local_course_id} (Canvas ID: {canvas_course_id}): {result}")
            continue
        ...
        for raw_assignment in raw_assignments:
            try:
                ...
                if description:
                    description = convert_html_to_markdown(description) # Keep HTML conversion

                # Prepare data for validation
                assignment_data = {
                    "id": raw_assignment.id, # Alias for canvas_assignment_id
                    "course_id": local_course_id,
                    "name": getattr(raw_assignment, "name", "Untitled Assignment"), # Alias for title
                    "description": description,
                    "assignment_type": assignment_type,
                    "due_at": getattr(raw_assignment, "due_at", None), # Alias for due_date
                    "unlock_at": getattr(raw_assignment, "unlock_at", None), # Alias for available_from
                    "lock_at": getattr(raw_assignment, "lock_at", None), # Alias for available_until
                ...
                logger.error(f"Validation error for assignment {getattr(raw_assignment, 'id', 'N/A')} in course {local_course_id}: {e}", exc_info=True)

    logger.info(f"Processed {total_raw_count} raw assignments, {len(all_valid_assignments)} valid assignments found.")
    ...
    logger.info(f"Finished assignment sync. Persisted/updated {persisted_count} assignments.")
    return persisted_count


async def _fetch_assignments_for_course(sync_service: "SyncService", canvas_course_id: int, local_course_id: int) -> list[Any] | None:
    ...
    async with sync_service.api_semaphore: # Limit concurrency here
        logger.debug(f"Semaphore acquired for fetching assignments: course {local_course_id}")
        try:
            ...
            logger.debug(f"Fetched {len(raw_assignments)} assignments for course {local_course_id}")
            return raw_assignments
        except Exception as e:
            logger.error(f"Error in thread fetching assignments for course {local_course_id}: {e}", exc_info=True)
...
def _persist_assignments(
    ...
    existing_assignments_map: dict[tuple[int, int], int] = {} # (canvas_assignment_id, course_id) -> local_assignment_id
    canvas_ids_in_batch = {a.canvas_assignment_id for a in valid_assignments}
    course_ids_in_batch = {a.course_id for a in valid_assignments} # Should usually be just one course_id if called per course, but handle multiple
    ...
    try:
        if canvas_ids_in_batch and course_ids_in_batch:
            ...
            for row in cursor.fetchall():
                existing_assignments_map[(row['canvas_assignment_id'], row['course_id'])] = row['id']
    ...
    for db_assignment in valid_assignments:
        ...
        assignment_dict["due_date"] = assignment_dict.pop("due_at", db_assignment.due_date)
        assignment_dict["available_from"] = assignment_dict.pop("unlock_at", db_assignment.available_from)
        assignment_dict["available_until"] = assignment_dict.pop("lock_at", db_assignment.available_until)
        ...
        if key in existing_assignments_map:
            ...
            assignment_dict["local_id"] = local_id # Add local_id for update WHERE clause
    ...
    if assignments_to_insert_data:
        ...
        try:
            ...
            for i, data_tuple in enumerate(assignments_to_insert_data):
                ...
                inserted_assignment_ids[canvas_id] = new_local_id
                assignment_local_id_map[canvas_id] = new_local_id # Add to map for calendar events
        except sqlite3.IntegrityError as e:
             # Handle potential unique constraint violations if UNIQUE index is added
             logger.warning(f"Integrity error during batch assignment insert (likely duplicate): {e}")
    ...
    if assignments_to_update_data:
        logger.debug(f"Updating {len(assignments_to_update_data)} assignments individually...")
        for item_dict in assignments_to_update_data:
            ...
            try:
                set_clause = ", ".join([f"{k} = ?" for k in item_dict if k not in ['canvas_assignment_id', 'course_id']])
                values = [v for k, v in item_dict.items() if k not in ['canvas_assignment_id', 'course_id']]
                ...
                logger.error(f"Failed to update assignment {canvas_id} (local ID {local_id}) in course {course_id}: {e}")
    ...
    events_to_persist = []
    for db_assignment in valid_assignments:
        local_assignment_id = assignment_local_id_map.get(db_assignment.canvas_assignment_id)
    ...
    if events_to_persist:
        logger.debug(f"Persisting {len(events_to_persist)} calendar events using INSERT OR REPLACE...")
