...
async def sync_conversations(
    sync_service: "SyncService", sync_days: int = DEFAULT_SYNC_DAYS
) -> int:
    """
    ...
    try:
        ...
        course_name_to_id = {
            row["course_name"]: row["id"] for row in cursor_map.fetchall()
        }
    ...
    try:
        ...
        async with sync_service.api_semaphore:
            raw_conversations = await asyncio.to_thread(
                ...
                per_page=100,  # Fetch summaries with pagination
            )
    except Exception as e:
    ...
    for raw_conv in raw_conversations:
        try:
            ...
            task = asyncio.create_task(
                _fetch_conversation_detail(sync_service, conv_id)
            )
            ...
            logger.error(
                f"Error pre-processing conversation summary {getattr(raw_conv, 'id', 'N/A')}: {e}",
                exc_info=True,
            )

    logger.info(
        f"Prepared {len(detail_fetch_tasks)} tasks to fetch conversation details."
    )

    # --- Parallel Fetch Stage (Details) ---
    detail_results_or_exceptions = await asyncio.gather(
    ...
    conv_ids_processed = list(
        valid_conv_summaries.keys()
    )  # Get IDs in the order tasks were created
    ...
    for i, result in enumerate(detail_results_or_exceptions):
        ...
        if isinstance(result, Exception):
            logger.error(
                f"Failed fetching details for conversation {conv_id}: {result}"
            )
            continue
        if result is None or not hasattr(result, "messages") or not result.messages:
            logger.warning(
                f"No valid messages found in details for conversation {conv_id}"
            )
            continue

        conv_detail = result
        message = conv_detail.messages[0]  # Get the most recent message
        message_created_at = _get_conversation_timestamp(
            raw_conv_summary, message
        )  # Get best timestamp

        # Final check on date (in case summary date was missing/wrong)
        if not message_created_at or message_created_at < cutoff_date:
            logger.debug(
                f"Skipping conv {conv_id} after detail fetch: date {message_created_at} older than cutoff {cutoff_date}"
            )
            continue
        ...
        try:
            ...
            if message_body:
                message_body = convert_html_to_markdown(
                    message_body
            ...
            logger.error(
                f"Validation error for conversation {conv_id}: {e}", exc_info=True
            )

    logger.info(
        f"Processed details, {len(all_valid_conversations)} valid conversations found."
    )
    ...
    logger.info(
        f"Finished conversation sync. Persisted/updated {persisted_count} conversations."
    )
    return persisted_count


async def _fetch_conversation_detail(
    sync_service: "SyncService", conv_id: int
) -> Any | None:
    ...
    async with sync_service.api_semaphore:
        ...
        try:
            detail = await asyncio.to_thread(
                sync_service.api_adapter.get_conversation_detail_raw, conv_id
            )
            return detail
        except Exception as e:
            logger.error(
                f"Error in thread fetching detail for conversation {conv_id}: {e}",
                exc_info=True,
            )
            return None


def _get_conversation_timestamp(
    raw_conv: Any, message: Any | None = None
) -> datetime | None:
    ...
    # 3. Fallback to conversation workflow_state change date (less ideal)
        ...
        hasattr(raw_conv, "workflow_state")
        and hasattr(raw_conv, "updated_at")
        and raw_conv.updated_at
    ):
    ...
    if timestamp_str:
        try:
            ...
            logger.warning(
                f"Could not parse timestamp string: {timestamp_str} for conv {getattr(raw_conv, 'id', 'N/A')}"
            )

    logger.warning(
        f"Could not determine timestamp for conversation {getattr(raw_conv, 'id', 'N/A')}"
    )
    return None
...
def _persist_conversations(
    ...
    if update_data:
        ...
        for item_dict in update_data:
            ...
            try:
                set_clause = ", ".join(
                    [f"{k} = ?" for k in item_dict if k != "canvas_conversation_id"]
                )
                values = [
                    v for k, v in item_dict.items() if k != "canvas_conversation_id"
                ]
                ...
                logger.error(
                    f"Failed to update conversation {canvas_id} (local ID {local_id}): {e}"
                )
