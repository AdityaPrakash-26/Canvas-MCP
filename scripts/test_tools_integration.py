...
# Import tools
from src.canvas_mcp.tools.assignments import get_course_assignments, get_upcoming_deadlines
...
def test_tool_chain(ctx):
    ...
    syllabus = get_syllabus(ctx, course_id)
    logger.info(f"Got syllabus for course {course_id}")
    
    # Step 7: Search course content
    logger.info("Step 7: Searching course content...")
...
def test_tool_performance(ctx):
    ...
    end_time = time.time()
    logger.info(f"get_course_assignments: 10 calls in {end_time - start_time:.2f} seconds")
    ...
    end_time = time.time()
    logger.info(f"search_course_content: 10 calls in {end_time - start_time:.2f} seconds")
...
def main():
    ...
    logger.info(f"Tool error handling test: {'OK' if tool_error_handling_ok else 'FAILED'}")
