    ...
    try:
        ...
        if violations:
            ...
            for violation in violations:
                logger.error(
                    f"  Table: {violation['table']}, Row ID: {violation['rowid']}"
                )
            ...
            if fix:
                ...
                confirm = input(
                    "This will delete records that violate foreign key constraints. Continue? (y/n): "
                )
                if confirm.lower() == "y":
                    for violation in violations:
                    ...
                    conn.commit()
                    logger.info(
                        "Fixed foreign key violations by deleting violating records"
                    )
                else:
    ...
    try:
        ...
        for constraint in unique_constraints:
            ...
            if duplicates:
                all_clean = False
                logger.error(
                    f"Found {len(duplicates)} sets of duplicate records in {constraint['table']} based on {constraint['description']}"
                )
                ...
                for dup in duplicates:
                    ...
                    cursor.execute(
                        f"SELECT * FROM {constraint['table']} WHERE {where_str}", params
                    )
                    ...
                    if fix:
                        ...
                        cursor.execute(
                            f"DELETE FROM {constraint['table']} WHERE {where_str} AND id != ?",
                            params + [keep_id],
                        )
                        logger.info(
                            f"  Kept record with ID {keep_id} and deleted {dup['count'] - 1} duplicates"
                        )
                ...
                logger.info(
                    f"No duplicate records found in {constraint['table']} based on {constraint['description']}"
                )
    ...
    try:
        ...
        for field_info in critical_fields:
            ...
            if null_count > 0:
                all_clean = False
                logger.error(
                    f"Found {null_count} records in {field_info['table']} with NULL {field_info['description']}"
                )
                ...
                logger.error(
                    f"  Sample record IDs: {', '.join(str(s['id']) for s in samples)}"
                )
            else:
                logger.info(
                    f"No NULL values found for {field_info['description']} in {field_info['table']}"
                )
...
def check_data_consistency(db_manager):
    ...
    try:
        ...
        if courses_no_assignments:
            logger.warning(
                f"Found {len(courses_no_assignments)} courses with no assignments"
            )
        ...
        if modules_no_items:
            ...
            for module in modules_no_items:
                logger.warning(
                    f"  Module {module['name']} in course {module['course_code']} has no items"
                )
        else:
...
def main():
    ...
    parser.add_argument(
        "--database", default="data/canvas_mcp.db", help="Path to the database file"
    )
    parser.add_argument(
    ...
    logger.info(
        f"Foreign key constraints: {'OK' if foreign_keys_ok else 'ISSUES FOUND'}"
    )
    logger.info(f"Orphaned records: {'OK' if orphaned_records_ok else 'ISSUES FOUND'}")
    logger.info(
        f"Duplicate records: {'OK' if duplicate_records_ok else 'ISSUES FOUND'}"
    )
    logger.info(
        f"NULL values in critical fields: {'OK' if null_values_ok else 'ISSUES FOUND'}"
    )
    logger.info(f"Data consistency: {'OK' if data_consistency_ok else 'ISSUES FOUND'}")

    if not all(
        [foreign_keys_ok, orphaned_records_ok, duplicate_records_ok, null_values_ok]
    ):
