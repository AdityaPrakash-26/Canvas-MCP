...
def run_test_script(script_path, args=None):
    ...
    if result.returncode == 0:
        logger.info(f"✅ {script_path} completed successfully in {elapsed_time:.2f} seconds")
    else:
        logger.error(f"❌ {script_path} failed with exit code {result.returncode} in {elapsed_time:.2f} seconds")
...
def run_pytest(test_path, args=None):
    ...
    if result.returncode == 0:
        logger.info(f"✅ pytest {test_path} completed successfully in {elapsed_time:.2f} seconds")
    else:
        logger.error(f"❌ pytest {test_path} failed with exit code {result.returncode} in {elapsed_time:.2f} seconds")
...
def generate_report(results):
    ...
    report_path = f"canvas_mcp_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
...
def main():
    ...
    tests = [
        ...
        {"type": "script", "path": "scripts/test_full_sync_process.py", "args": [f"--term_id={args.term_id}"]},
        {"type": "script", "path": "scripts/check_database_relationships.py", "args": ["--fix"] if args.fix else []},
