...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f"sync_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"),
    ],
)
...
def setup_test_environment():
    ...
    if not api_key or not api_url:
        logger.error("Canvas API credentials not found. Please set CANVAS_API_KEY and CANVAS_API_URL environment variables.")
        sys.exit(1)
...
def test_api_connectivity(api_adapter):
    ...
    logger.info(f"Successfully connected to Canvas API as user: {user.name} (ID: {user.id})")
    return True
...
def test_database_setup(db_manager):
    ...
    try:
        ...
        expected_tables = [
            "courses", "assignments", "modules", "module_items", 
            "announcements", "syllabi", "user_courses"
        ]
...
def perform_sync_with_timing(sync_service, term_id):
    ...
    if not course_ids:
        logger.warning("No courses were synced. Check term_id parameter and Canvas access.")
        return None
    ...
    logger.info(f"Synced {assignment_count} assignments in {assignment_time:.2f} seconds")
    ...
    logger.info(f"Synced {announcement_count} announcements in {announcement_time:.2f} seconds")
...
def verify_data_integrity(db_manager, sync_results):
    ...
    try:
        ...
        if course_count != sync_results["courses"]:
            logger.warning(f"Course count mismatch: {course_count} in DB vs {sync_results['courses']} reported")
        ...
        if assignment_count != sync_results["assignments"]:
            logger.warning(f"Assignment count mismatch: {assignment_count} in DB vs {sync_results['assignments']} reported")
        
        # Check module count
        cursor.execute("SELECT COUNT(*) as count FROM modules")
        module_count = cursor.fetchone()["count"]
        if module_count != sync_results["modules"]:
            logger.warning(f"Module count mismatch: {module_count} in DB vs {sync_results['modules']} reported")
        ...
        if announcement_count != sync_results["announcements"]:
            logger.warning(f"Announcement count mismatch: {announcement_count} in DB vs {sync_results['announcements']} reported")
        ...
        cursor.execute("SELECT COUNT(*) as count FROM courses WHERE course_code IS NULL OR name IS NULL")
...
def check_edge_cases(db_manager):
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(*) as count FROM assignments WHERE length(description) > 10000")
        long_descriptions = cursor.fetchone()["count"]
        if long_descriptions > 0:
            logger.warning(f"Found {long_descriptions} assignments with very long descriptions")
        
        # Check for unusual dates
        cursor.execute("SELECT COUNT(*) as count FROM assignments WHERE due_date < '2000-01-01' OR due_date > '2100-01-01'")
        ...
        if duplicate_assignments:
            logger.error(f"Found {len(duplicate_assignments)} duplicate assignment Canvas IDs")
            for row in duplicate_assignments:
                logger.error(f"  Canvas ID {row['canvas_assignment_id']} appears {row['count']} times")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test the full synchronization process")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging")
    parser.add_argument("--term_id", type=int, default=-1, help="Term ID to sync (default: -1, most recent term)")
